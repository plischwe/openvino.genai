...
EncodedImage VisionEncoderMiniCPM::encode(const ov::Tensor& image, const ov::AnyMap& config_map) {
    ...
    auto clip_setup_duration = std::chrono::duration_cast<std::chrono::milliseconds>(clip_setup_end - clip_setup_start);
    std::cout << "CLIP setup time: " << clip_setup_duration.count() << " ms" << std::endl;
